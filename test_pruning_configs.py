#!/usr/bin/env python3
"""
Test 5 pruning configurations: baseline, light, moderate, aggressive, extreme
Total time: ~5 hours (1 hour each)
"""
import os
import sys
import subprocess
...
# Define 3 specific configurations with ALL parameters (2 commented out)
CONFIGS = [
    {
        "name": "baseline",
        "description": "Original settings (reference)",
        "min_opacity": 0.05,
        "max_screen_size": 20,
    ...
    },
    {
        ...
        "description": "Moderate pruning (~25% reduction target)",
        "min_opacity": 0.10,
        "max_screen_size": 17,
    ...
    },
    {
        ...
        "description": "Aggressive pruning (~30% reduction target)",
        "min_opacity": 0.13,
        "max_screen_size": 16,
...
def save_results_csv(results, output_base, baseline_count):
    """Save results to CSV file"""
    csv_path = os.path.join(output_base, f'pruning_results_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv')
    ...
    with open(csv_path, 'w', newline='') as f:
        ...
        for result in results:
            if result['count'] is not None:
                reduction_pct = ""
                if baseline_count and result['name'] != 'baseline':
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python test_pruning_configs.py <dataset_path> [output_base] [--quick]")
        print("\nExample:")
        print("  python test_pruning_configs.py ../RaDe-GS_DATA/dtu_preproc/2DGS_data/DTU/scan105 ../RaDe-GS_RUNS/pruning_test")
        print("  python test_pruning_configs.py ../RaDe-GS_DATA/dtu_preproc/2DGS_data/DTU/scan105 ../RaDe-GS_RUNS/pruning_test --quick")
        print(f"\nThis will run {len(CONFIGS)} configurations")
        print("  Normal mode: 30k iterations (~2 hours each = ~10 hours total)")
        print("  Quick mode: 16k iterations (~1 hour each = ~5 hours total)")
        sys.exit(1)
    
    dataset_path = sys.argv[1]
    quick_mode = '--quick' in sys.argv
    output_base = sys.argv[2] if len(sys.argv) > 2 and sys.argv[2] != '--quick' else '../RaDe-GS_RUNS/pruning_test'
    iterations = 16000 if quick_mode else 30000
    ...
    print(f"\n{'='*80}")
    print("PRUNING CONFIGURATION TEST")
    print(f"{'='*80}")
    print(f"Dataset: {dataset_path}")
    print(f"Output: {output_base}")
    print(f"Mode: {'QUICK (16k iter)' if quick_mode else 'NORMAL (30k iter)'}")
    print(f"Configurations to test: {len(CONFIGS)}")
    est_hours = len(CONFIGS) if quick_mode else len(CONFIGS) * 2
    ...
    # Print table
    print(f"{'Config':<20} {'Gaussians':>12} {'Reduction':>12} {'Opacity':>10} {'Screen':>8} {'Grad':>10}")
    print(f"{'-'*20} {'-'*12} {'-'*12} {'-'*10} {'-'*8} {'-'*10}")
    ...
    for result in results:
        if result['count'] is not None:
            ...
            print(f"{result['name']:<20} {result['count']:>12,} {reduction:>12} "
                  f"{result['config']['min_opacity']:>10.2f} {result['config']['max_screen_size']:>8} "
                  f"{result['config']['densify_grad_threshold']:>10.5f}")
        else:
            print(f"{result['name']:<20} {'FAILED':>12}")
    
    print()
    
    if baseline_count:
        target = int(baseline_count * 0.7)
        print(f"Target for 30% reduction: ~{target:,} Gaussians\n")
    
    # Save to CSV
    csv_path = save_results_csv(results, output_base, baseline_count)
