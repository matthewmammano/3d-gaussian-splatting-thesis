...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    @staticmethod
    def backward(
        ctx, grad_color, grad_radii, grad_coord, grad_mcoord, grad_depth, grad_mdepth, grad_alpha, grad_normal
    ):
...
class GaussianRasterizer(nn.Module):
    ...
    def quantify_error(
        self, means3D, colors_precomp, rendered_color, geomBuffer, num_rendered, binningBuffer, imgBuffer
    ):
    ...
    def integrate(
        ...
        subpixel_offset = torch.zeros(
            (int(raster_settings.image_height), int(raster_settings.image_width), 2), dtype=torch.float32, device="cuda"
        )
        # Invoke C++/CUDA rasterization routine
        ...
        if raster_settings.debug:
            cpu_args = cpu_deep_copy_tuple(args)  # Copy them before they can be corrupted
            try:
                num_rendered, color, alpha_integrated, color_integrated, radii, geomBuffer, binningBuffer, imgBuffer = (
