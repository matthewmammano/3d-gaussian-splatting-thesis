...
@torch.no_grad()
def quantify_gaussianpop_error(
    viewpoint_camera, pc: GaussianModel, pipe, bg_color: torch.Tensor, kernel_size, scaling_modifier=1.0
):
    tanfovx = math.tan(viewpoint_camera.FoVx * 0.5)
    tanfovy = math.tan(viewpoint_camera.FoVy * 0.5)
    ...
    num_rendered, rendered_image, _, _, _, _, _, _, radii, geom_buffer, binning_buffer, img_buffer = (
        rasterizer.forward_with_aux(
            means3D=means3D,
            means2D=screenspace_points,
            shs=shs,
            colors_precomp=colors_precomp,
